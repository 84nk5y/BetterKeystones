...
local function GetTimerFrame(block)
    if not block.TimerFrame then
        ...
        local barWidth = block.StatusBar:GetWidth()
        TimerFrame.Bar3 = TimerFrame:CreateTexture(nil, "OVERLAY")
        TimerFrame.Bar3:SetPoint("TOPLEFT", block.StatusBar, "TOPLEFT", barWidth * (1 - TIME_FOR_3) - 4, 0)
        ...
        TimerFrame.Bar2:SetPoint("TOPLEFT", block.StatusBar, "TOPLEFT", barWidth * (1 - TIME_FOR_2) - 4, 0)
...
local function UpdateTime(block, elapsedTime)
    ...
    if elapsedTime < time3 then
        TimerFrame.Text:SetText(FormatTime(time3 - elapsedTime))
        TimerFrame.Text:SetTextColor(1, 0.843, 0)
        TimerFrame.Text:Show()
    elseif elapsedTime < time2 then
        TimerFrame.Text:SetText(FormatTime(time2 - elapsedTime))
    ...
    if elapsedTime > block.timeLimit then
        block.TimeLeft:SetText(FormatTime(elapsedTime - block.timeLimit))
    end
end
